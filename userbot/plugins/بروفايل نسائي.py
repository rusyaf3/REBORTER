...
DEFAULTUSERBIO = DEFAULT_BIO or " كن لنفسك بتفكيرك و عملك .! "
...
@jmthon.ar_cmd(
    pattern="صوره نسائيه$",
    command=("صوره نسائيه", plugin_category),
)
async def _(event):
...
@jmthon.ar_cmd(
    pattern="بشرفك لانكحك$",
    command=("بشرفك لانكحك", plugin_category),
)
async def _(event):
...
@jmthon.ar_cmd(
    pattern="لا تخمط ولاك$",
    command=("لا تخمط ولاك", plugin_category),
)
async def _(event):
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str == "صوره نسائيه":
        if gvarstatus("digitalpic") is not None and gvarstatus("digitalpic") == "true":
            ...
            return await edit_delete(event, "**تم ايقاف صوره نسائيه بنـجاح ✅**")
        return await edit_delete(event, "**لم يتم تفعيل صوره نسائيه بالأصل 🧸♥**")
    if input_str == "بشرفك لانكحك":
        if gvarstatus("autoname") is not None and gvarstatus("autoname") == "true":
            delgvar("autoname")
            await event.client(
